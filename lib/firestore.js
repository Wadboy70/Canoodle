...
export const complexQuery = async (whereClauses, ...collectionPath) => {
  const collectionRef = collection(db, ...collectionPath);
...
export const getdisplayRecipeImg = async (listId) => {
  const displayRelation = await simpleQuery(
    "list_id",
    listId,
    COLLECTION_NAMES.RECIPE_LISTS
  );

  if (!displayRelation || !displayRelation.length) return null;

  const displayRecipe = await getSingleFirestoreDoc(
    COLLECTION_NAMES.RECIPE_DATA,
    displayRelation[0].recipe_id
  );

  return displayRecipe.image;
};
